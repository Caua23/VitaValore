...
public interface VendasRepository extends CrudRepository<Vendas, Long> {
    ...
    @Query("SELECT YEAR(v.dataVenda) as ano, MONTH(v.dataVenda) as mes, DAY(v.dataVenda) as dia, SUM(v.quantidade) as totalItensVendidos " +
            "FROM Vendas v " +
    ...
    List<Object[]> findItensVendidosUltimosTresMeses(@Param("empresaId") Long empresaId, @Param("dataInicio") LocalDate dataInicio);
